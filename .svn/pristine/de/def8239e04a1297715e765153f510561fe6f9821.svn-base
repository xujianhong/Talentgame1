...
constructor(model: HomeContract.Model, view: HomeContract.View) :
    ...
    fun homePictureSet(){
        mModel.homePictureSet()
            .retryWhen(RetryWithDelay(2, 5))
            .compose(RxUtils.applySchedulers(mRootView))
            .subscribe(object :ErrorHandleSubscriber<BaseJson<HomePictureSet>>(mErrorHandler){
                override fun onNext(t: BaseJson<HomePictureSet>) {
                    if (Api.SUCCESS == t.code) {
                        mRootView.requestHomePictureSetSuccess(t.data)
                    } else {
                        mRootView.showMessage(t.msg)
                    }
                }
